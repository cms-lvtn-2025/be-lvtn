...
func Load() (*Config, error) {
	...
	envPaths := []string{
		"/home/thaily/code/lvtn/BE_main/src/server/.server.env", // Current directory
	}
	...
	cfg := &Config{
		...
		Services: ServiceMap{
			Academic: ServiceConfig{
				Port:     getEnv("ACADEMIC_PORT", "50051"),
				Endpoint: getEnv("SERVICE_ACADEMIC_URL", "academic"),
			},
			Council: ServiceConfig{
				Port:     getEnv("COUNCIL_PORT", "50052"),
				Endpoint: getEnv("SERVICE_COUNCIL_URL", "council"),
			},
			File: ServiceConfig{
				Port:     getEnv("FILE_PORT", "50053"),
				Endpoint: getEnv("SERVICE_FILE_URL", "file"),
			},
			Role: ServiceConfig{
				Port:     getEnv("ROLE_PORT", "50054"),
				Endpoint: getEnv("SERVICE_ROLE_URL", "role"),
			},
			Thesis: ServiceConfig{
				Port:     getEnv("THESIS_PORT", "50055"),
				Endpoint: getEnv("SERVICE_THESIS_URL", "thesis"),
			},
			User: ServiceConfig{
				Port:     getEnv("USER_PORT", "50056"),
				Endpoint: getEnv("SERVICE_USER_URL", "user"),
			},
			MinIo: MinioConfig{
				Endpoint:   os.Getenv("MIN_ENDPOINT"),
