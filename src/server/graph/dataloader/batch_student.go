	...
	return func(ctx context.Context, ids []string) (map[string]*model.StudentCouncil, error) {
		...
		if err != nil {
			...
			for _, id := range ids {
				...
				if council != nil && council.GetCouncil() != nil {
					result[id] = convert2.PbCouncilToStudentCouncil(council.GetCouncil())
				}
			}
		}
		if resp != nil && resp.Councils != nil {
			for _, council := range resp.Councils {
				if council != nil {
					result[council.Id] = convert2.PbCouncilToStudentCouncil(council)
				}
			}
		}
		return result, nil
		...
		if err != nil {
			...
			for _, id := range ids {
				...
				if topicCouncil != nil && topicCouncil.GetTopicCouncil() != nil {
					result[id] = convert2.PbTopicCouncilToStudentTopicCouncil(topicCouncil.GetTopicCouncil())
				}
			}
		}
		if resp != nil && resp.TopicCouncils != nil {
			for _, pbTopicCouncil := range resp.TopicCouncils {
				if pbTopicCouncil != nil {
					result[pbTopicCouncil.Id] = convert2.PbTopicCouncilToStudentTopicCouncil(pbTopicCouncil)
				}
			}
		}
	...
	return func(ctx context.Context, ids []string) (map[string]*model.StudentTopic, error) {
		...
		if err != nil {
			...
			for _, id := range ids {
				...
				if topic != nil && topic.Topic != nil {
					result[id] = convert2.PbTopicToStudentTopic(topic.Topic)
				}
			}
			log.Printf("[DataLoader] Individual fetch completed: %d/%d successful", len(result), len(ids))
			return result, nil
		...
		if resp != nil && resp.Topics != nil {
			for _, pbTopic := range resp.Topics {
				if pbTopic != nil {
					result[pbTopic.Id] = convert2.PbTopicToStudentTopic(pbTopic)
				}
			}
		}
		...
		if err != nil {
			...
			for _, id := range ids {
				...
				if gradeReview != nil && gradeReview.GetGradeReview() != nil {
					result[id] = convert2.PbGradeReviewToModel(gradeReview.GetGradeReview())
				}
			}
		}
		if resp != nil && resp.GradeReviews != nil {
			for _, pbGradeReview := range resp.GradeReviews {
				if pbGradeReview != nil {
					result[pbGradeReview.Id] = convert2.PbGradeReviewToModel(pbGradeReview)
				}
			}
		}
	...
	return func(ctx context.Context, ids []string) (map[string][]*model.StudentTopicSupervisor, error) {
		...
		if err != nil {
			...
			for _, id := range ids {
				...
				if supervisor != nil {
					result[id] = convert2.PbTopicCouncilSupervisorsToStudentTopicSupervisors(supervisor.GetTopicCouncilSupervisors())
				}
			}
			return result, err
		}
		if resp != nil {
			for _, supervisor := range resp.GetTopicCouncilSupervisors() {
				if supervisor != nil {
					result[supervisor.TopicCouncilCode] = append(result[supervisor.TopicCouncilCode], convert2.PbTopicCouncilSupervisorToStudentTopicSupervisor(supervisor))
				}
			}
		}
		return result, err
