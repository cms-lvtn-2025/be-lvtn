package resolver

// This file will be automatically regenerated based on the schema, any resolver
// implementations
// will be copied through when generating and any unknown code will be moved to the end.
// Code generated by github.com/99designs/gqlgen version v0.17.84

import (
	"context"
	"thaily/src/server/graph/dataloader"
	"thaily/src/server/graph/generated"
	"thaily/src/server/graph/model"
)

// Student is the resolver for the student field.
func (r *enrollmentResolver) Student(ctx context.Context, obj *model.Enrollment) (*model.Student, error) {
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.StudentById != nil {
		return loaders.StudentById.Load(ctx, obj.StudentCode)
	}
	return r.Ctrl.GetStudentById(ctx, obj.StudentCode)
}

// Midterm is the resolver for the midterm field.
func (r *enrollmentResolver) Midterm(ctx context.Context, obj *model.Enrollment) (*model.Midterm, error) {
	if obj.MidtermCode == nil {
		return nil, nil
	}
	access, err := r.Resolver.RbacAccess(ctx, []string{"supervisor", "department", "affair", "student"})
	if err != nil {
		return nil, err
	}
	if !access {
		return nil, nil
	}
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.MidtermByID != nil {
		midterm, err := loaders.MidtermByID.Load(ctx, *obj.MidtermCode)
		if err != nil {
			return nil, err
		}
		return midterm, nil
	}

	return r.Ctrl.GetMidtermById(ctx, *obj.MidtermCode)
}

// Final is the resolver for the final field.
// supervisor, department, affair, student
func (r *enrollmentResolver) Final(ctx context.Context, obj *model.Enrollment) (*model.Final, error) {
	if obj.FinalCode == nil {
		return nil, nil
	}
	access, err := r.Resolver.RbacAccess(ctx, []string{"supervisor", "department", "affair", "student"})
	if err != nil {
		return nil, err
	}
	if !access {
		return nil, nil
	}

	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.FinalByID != nil {
		final, err := loaders.FinalByID.Load(ctx, *obj.FinalCode)
		if err != nil {
			return nil, err
		}
		return final, nil
	}

	return r.Ctrl.GetFinalById(ctx, *obj.FinalCode)
}

// TopicCouncil is the resolver for the topicCouncil field.
// student
func (r *enrollmentResolver) TopicCouncil(ctx context.Context, obj *model.Enrollment) (*model.TopicCouncil, error) {
	access, err := r.Resolver.RbacAccess(ctx, []string{"student"})
	if err != nil {
		return nil, err
	}
	if !access {
		return nil, nil
	}
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.TopicCouncilById != nil {
		return loaders.TopicCouncilById.Load(ctx, obj.TopicCouncilCode)
	}
	return r.Ctrl.GetTopicCouncilById(ctx, obj.TopicCouncilCode)
}

// GradeReview is the resolver for the gradeReview field.
// delete
// delete
func (r *enrollmentResolver) GradeReview(ctx context.Context, obj *model.Enrollment) (*model.GradeReview, error) {
	if obj.GradeReviewCode == nil {
		return nil, nil
	}

	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.GradeViewById != nil {
		return loaders.GradeViewById.Load(ctx, *obj.GradeReviewCode)
	}

	return r.Ctrl.GetGradeReviewById(ctx, *obj.GradeReviewCode)
}

// GradeDefences is the resolver for the gradeDefences field.
// supervisor, department, student, affair
func (r *enrollmentResolver) GradeDefences(ctx context.Context, obj *model.Enrollment) ([]*model.GradeDefence, error) {
	access, err := r.Resolver.RbacAccess(ctx, []string{"supervisor", "department", "student", "affair"})
	if err != nil {
		return nil, err
	}
	if !access {
		return nil, nil
	}

	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.GradeDefenceByEnrollmentId != nil {
		return loaders.GradeDefenceByEnrollmentId.Load(ctx, obj.ID)
	}
	return r.Ctrl.GetGradeDefencesByEnrollmentId(ctx, obj.ID)
}

// Teacher is the resolver for the teacher field.
// delete
// delete
func (r *gradeReviewResolver) Teacher(ctx context.Context, obj *model.GradeReview) (*model.StudentTeacherInfo, error) {
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.TeacherInfoById != nil {
		teacher, err := loaders.TeacherInfoById.Load(ctx, obj.TeacherCode)
		if err != nil {
			return nil, err
		}
		return teacher, nil
	}
	teacher, err := r.Ctrl.GetTeacherInfoById(ctx, obj.TeacherCode)
	if err != nil {
		return nil, err
	}
	return teacher, nil
}

// Files is the resolver for the files field.
func (r *topicResolver) Files(ctx context.Context, obj *model.Topic) ([]*model.File, error) {
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.FilesByTopicId != nil {
		return loaders.FilesByTopicId.Load(ctx, obj.ID)
	}
	return r.Ctrl.GetFilesByTopicId(ctx, obj.ID)
}

// TopicCouncils is the resolver for the topicCouncils field.
// for supervisor, department, affair
func (r *topicResolver) TopicCouncils(ctx context.Context, obj *model.Topic) ([]*model.TopicCouncil, error) {
	access, err := r.Resolver.RbacAccess(ctx, []string{"affair", "department", "supervisor"})
	if err != nil {
		return nil, err
	}
	if !access {
		return nil, nil
	}
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.TopicCouncilByTopicId != nil {
		return loaders.TopicCouncilByTopicId.Load(ctx, obj.ID)
	}
	return r.Ctrl.GetTopicCouncilsByTopicId(ctx, obj.ID)
}

// Topic is the resolver for the topic field.
func (r *topicCouncilResolver) Topic(ctx context.Context, obj *model.TopicCouncil) (*model.Topic, error) {
	access, err := r.Resolver.RbacAccess(ctx, []string{"affair", "department", "student", "supervisor", "council"})
	if err != nil {
		return nil, err
	}
	if !access {
		return nil, nil
	}
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.TopicByID != nil {
		topic, err := loaders.TopicByID.Load(ctx, obj.TopicCode)
		if err != nil {
			return nil, err
		}
		return topic, nil
	}

	return r.Ctrl.GetTopicById(ctx, obj.TopicCode)
}

// Council is the resolver for the council field.
// no council
func (r *topicCouncilResolver) Council(ctx context.Context, obj *model.TopicCouncil) (*model.Council, error) {
	if obj.CouncilCode == nil {
		return nil, nil
	}

	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.CouncilByID != nil {
		council, err := loaders.CouncilByID.Load(ctx, *obj.CouncilCode)
		if err != nil {
			return nil, err
		}
		return council, nil
	}

	return r.Ctrl.GetCouncilById(ctx, *obj.CouncilCode)
}

// Enrollments is the resolver for the enrollments field.
// no student
func (r *topicCouncilResolver) Enrollments(ctx context.Context, obj *model.TopicCouncil) ([]*model.Enrollment, error) {
	access, err := r.Resolver.RbacAccess(ctx, []string{"affair", "department", "council", "affair"})
	if err != nil {
		return nil, err
	}
	if !access {
		return nil, nil
	}

	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.EnrollmentByTopicCouncilId != nil {
		return loaders.EnrollmentByTopicCouncilId.Load(ctx, obj.ID)
	}
	return r.Ctrl.GetEnrollmentsByTopicCouncilId(ctx, obj.ID)
}

// Supervisors is the resolver for the supervisors field.
func (r *topicCouncilResolver) Supervisors(ctx context.Context, obj *model.TopicCouncil) ([]*model.TopicCouncilSupervisor, error) {
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.SupervisorsByTopicCouncilId != nil {
		return loaders.SupervisorsByTopicCouncilId.Load(ctx, obj.ID)
	}
	return r.Ctrl.GetSupervisorsByTopicCouncilId(ctx, obj.ID)
}

// Teacher is the resolver for the teacher field.
func (r *topicCouncilSupervisorResolver) Teacher(ctx context.Context, obj *model.TopicCouncilSupervisor) (*model.Teacher, error) {
	loaders := dataloader.GetLoaders(ctx)
	if loaders != nil && loaders.TeacherById != nil {
		teacher, err := loaders.TeacherById.Load(ctx, obj.TeacherSupervisorCode)
		if err != nil {
			return nil, err
		}
		return teacher, nil
	}

	return r.Ctrl.GetTeacherById(ctx, obj.TeacherSupervisorCode)
}

// Enrollment returns generated.EnrollmentResolver implementation.
func (r *Resolver) Enrollment() generated.EnrollmentResolver { return &enrollmentResolver{r} }

// GradeReview returns generated.GradeReviewResolver implementation.
func (r *Resolver) GradeReview() generated.GradeReviewResolver { return &gradeReviewResolver{r} }

// Topic returns generated.TopicResolver implementation.
func (r *Resolver) Topic() generated.TopicResolver { return &topicResolver{r} }

// TopicCouncil returns generated.TopicCouncilResolver implementation.
func (r *Resolver) TopicCouncil() generated.TopicCouncilResolver { return &topicCouncilResolver{r} }

// TopicCouncilSupervisor returns generated.TopicCouncilSupervisorResolver implementation.
func (r *Resolver) TopicCouncilSupervisor() generated.TopicCouncilSupervisorResolver {
	return &topicCouncilSupervisorResolver{r}
}

type enrollmentResolver struct{ *Resolver }
type gradeReviewResolver struct{ *Resolver }
type topicResolver struct{ *Resolver }
type topicCouncilResolver struct{ *Resolver }
type topicCouncilSupervisorResolver struct{ *Resolver }
